...
trait RequestSyntax[Req[Meth <: Method, Accept <: Coproduct, Content, ContentType]] { self: RequestTypes[Req] =>
  ...
  def head(
    url: URL,
    filters: Filter[Request, Response, Request, Response] = Filter.identity
  ): HeadRequest = req[Method.Head.type, CNil](Method.Head, url, filters = filters)
...
trait RequestTypes[Req[Meth <: Method, Accept <: Coproduct, Content, ContentType]] {

  type GetRequest[Accept <: Coproduct] = Req[Method.Get.type, Accept, None.type, None.type]
  ...
  type HeadRequest = Req[Method.Head.type, CNil, None.type, None.type]
  type DeleteRequest[Accept <: Coproduct] = Req[Method.Delete.type, Accept, None.type, None.type]

  type PatchRequest[Accept <: Coproduct, Content, ContentType] =
    Req[Method.Patch.type, Accept, Content, ContentType]
}
...
//                Either.left("Too many redirects; giving up")
//              else
//                Either.right(())
...
//            handleRequest(newReq, httpClient, numRedirects + 1)
//          }
//          attempt.fold(
...
//  ): Future[K] =
//    buildRequest match {
//      case Valid(req) => handleRequest(req, httpClient).flatMap { rep =>
...
//    case Valid(req) => handleRequest(req, httpClient)
//      .flatMap {
//        rep => decodeResponse[Success](rep).map(Either.right[Error, Success]).map((_, rep))
//      }.rescue {
//        case ErrorResponse(_, rep) => decodeResponse[Error](rep).map(Either.left[Error, Success]).map((_, rep))
//      }
//    case Invalid(errs) => Future.exception(RequestBuildingError(errs))
//  }
//
...
//    sendZipRequest[Error, Success](canBuild, decodeAllSuccess, decodeAllError, httpClient).map(_._1)
//
//}
//
...
//    decodeAll: DecodeAll[K, Accept]
//  ): Future[K] = sendRequest[K](canBuild, decodeAll, httpClient)
//
...
//    decodeAllSuccess: DecodeAll[Success, Accept]
//  ): Future[Either[Error, Success]] =
//    sendRequest[Error, Success](canBuild, decodeAllSuccess, decodeAllError, httpClient)
//
...
//    sendZipRequest[Error, Success](canBuild, decodeAllSuccess, decodeAllError, httpClient)
//}
//
...
//    first: (String, String),
//    rest: (String, String)*
//  ): FormPostRequest[Accept, FormRight] = {
